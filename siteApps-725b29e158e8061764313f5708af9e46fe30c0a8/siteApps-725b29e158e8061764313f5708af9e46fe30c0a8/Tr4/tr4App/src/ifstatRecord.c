...
epicsExportAddress(rset,ifstatRSET);

typedef struct ifstatset {
	long		number;
...
static void checkAlarms(ifstatRecord *prec);
...
static long init_record(void *precord,int pass)
{
    ...
    if(!(pdset = (ifstatdset *)(prec->dset))) {
	recGblRecordError(S_dev_noDSET,(void *)prec,"ifstat: init_record");
	return(S_dev_noDSET);
    }
    /* must have read_ifstat function defined */
    if( (pdset->number < 5) || (pdset->read_ifstat == NULL) ) {
	recGblRecordError(S_dev_missingSup,(void *)prec,"ifstat: init_record");
...
static long get_graphic_double(DBADDR *paddr,struct dbr_grDouble *pgd)
{
    ...
    || fieldIndex == ifstatRecordHOPR
    || fieldIndex == ifstatRecordLOPR) {
